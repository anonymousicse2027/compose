...
int main(int argc, char **argv, char **envp) {
  ...
  if (Watchdog) {
    ...
    if (pid<0) {
      ...
      const auto startTime = time::getWallTime();
      int level = 0;
      ...
      while (1) {
        ...
        if (res < 0) {
          ...
          auto elapsed = time::getWallTime() - startTime;
          if (WIFSIGNALED(status)) {
            int termSignal = WTERMSIG(status);
            if (termSignal == SIGTERM && elapsed.toSeconds() >= 10.0) {
              klee_message(
                  "KLEE: WATCHDOG: ReDos detected (signal %d after %.0fs)\n",
                  termSignal, elapsed.toSeconds());
            }
