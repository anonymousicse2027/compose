...
// ── Regex function-based state guiding options ──
// (moved outside anonymous namespace for cross-TU visibility)
...
// ── Regex function-based state guiding options ──
// Declared outside anonymous namespace for cross-TU visibility (UserSearcher.cpp)
cl::opt<bool> GuideByRegex(
    "guide-by-regex",
    cl::desc("Guide state exploration based on regex function reachability. "
             "States that have not reached regex functions (regcomp/regexec/"
             "re_search) are pruned after a threshold (default=false)"),
    cl::init(false));

// ── Regex source files for --guide-by-regex ──
// The regex function set is every function DEFINED in one of these regex
// source files (matched by basename against debug-info file names). No caller
// expansion is performed.
cl::opt<std::string> RegexSrcFiles(
    "regex-src-files",
    cl::desc("Comma-separated basenames of regex source files whose defined "
             "functions form the regex function set for --guide-by-regex "
             "(matched against debug-info file names)"),
    cl::init("regex_internal.c,regex_internal.h,regcomp.c,regexec.c,dfa.c,dfasearch.c"));

static std::set<std::string> regexSrcFileSet;
static bool regexSrcFileSetParsed = false;
static void parseRegexSrcFiles() {
  if (regexSrcFileSetParsed) return;
  regexSrcFileSetParsed = true;
  const std::string &s = RegexSrcFiles;
  size_t start = 0;
  while (start <= s.size()) {
    ...
    if (a != std::string::npos) regexSrcFileSet.insert(tok.substr(a, b - a + 1));
    if (comma == std::string::npos) break;
    start = comma + 1;
  }
}

static std::string regexBaseName(const std::string &path) {
  size_t p = path.find_last_of("/\\");
  return (p == std::string::npos) ? path : path.substr(p + 1);
}

// True if F is *defined* in one of the configured regex source files.
static bool isInRegexSrcFile(const llvm::Function &F) {
  if (F.isDeclaration()) return false;
  if (llvm::DISubprogram *sp = F.getSubprogram()) {
    if (llvm::DIFile *file = sp->getFile())
      return regexSrcFileSet.count(regexBaseName(file->getFilename().str())) > 0;
  }
  return false;
}

// ── CompoSE: Auto-detected regex function set ──
static std::set<std::string> regexFunctionSet;
static bool regexFunctionSetBuilt = false;

/// Build the regex function set:
///   Seed = every non-declaration function physically DEFINED in one of the
///   configured regex source files (--regex-src-files). No caller expansion.
static void buildRegexFunctionSet(llvm::Module *module) {
  if (regexFunctionSetBuilt || !module) return;
  regexFunctionSetBuilt = true;

  parseRegexSrcFiles();
  unsigned fileCount = 0;

  for (auto &F : *module) {
    if (F.isDeclaration() && F.use_empty()) continue;
    if (isInRegexSrcFile(F)) {
      regexFunctionSet.insert(F.getName().str());
      fileCount++;
    }
  }

  klee_message("CompoSE: auto-detected %lu regex functions (%u src-file)",
               (unsigned long)regexFunctionSet.size(), fileCount);
  for (const auto &fn : regexFunctionSet)
    klee_message("CompoSE:   %s", fn.c_str());
}
...
void Executor::executeCall(ExecutionState &state, 
  ...
  // ── CompoSE: Regex function reachability detection (auto-detected) ──
  if (GuideByRegex && f && !regexFunctionSet.empty()) {
    std::string fname = f->getName().str();
    if (regexFunctionSet.count(fname)) {
      state.reachedRegexCompile = true;
      state.reachedRegexFunctions.insert(fname);
    }
  }
...
void Executor::run(ExecutionState &initialState) {
  ...
  // CompoSE: Auto-detect regex functions from the LLVM module
  if (GuideByRegex) {
    buildRegexFunctionSet(kmodule->module.get());
  }
  ...
  if (usingSeeds) {
    ...
    // CompoSE: After seed replay, remove phase-0 states (states that never
    // reached any regex function). Only seed-replay siblings are affected;
    // during normal execution, new phase-0 states are NOT filtered.
    if (GuideByRegex) {
      std::vector<ExecutionState*> seedPhase0;
      for (auto *es : states) {
        if (!es->reachedRegexCompile && !es->reachedRegexMatch) {
          seedPhase0.push_back(es);
        }
      }
      if (!seedPhase0.empty() && seedPhase0.size() < states.size()) {
        for (auto *es : seedPhase0) {
          terminateStateEarly(*es, "CompoSE: seed-replay phase-0 state removed.");
        }
        updateStates(nullptr);
        klee_message("CompoSE: removed %lu phase-0 seed states, %lu remain",
                     (unsigned long)seedPhase0.size(), (unsigned long)states.size());
      } else if (!seedPhase0.empty()) {
        klee_message("CompoSE: all %lu seed states are phase-0, keeping all (fallback)",
                     (unsigned long)states.size());
      } else {
        klee_message("CompoSE: no phase-0 seed states to remove");
      }
    }
...
static unsigned constTestCaseCounter = 1;
...
bool Executor::getSymbolicSolution(const ExecutionState &state,
  ...
  // CompoSE: Write .regex_data file with regex reachability info
  if (GuideByRegex) {
    std::ostringstream idStream;
    idStream << std::setw(6) << std::setfill('0') << constTestCaseCounter;
    std::string testId = idStream.str();

    std::string outDir = interpreterHandler->getOutputFilename("test" + testId + ".ktest");
    outDir = outDir.substr(0, outDir.find_last_of('/'));
    std::string filename = outDir + "/test" + testId + ".regex_data";

    std::ofstream rdFile(filename);
    if (rdFile.is_open()) {
      rdFile << "regex_compile_reached="
             << (state.reachedRegexCompile ? "true" : "false") << "\n";
      rdFile << "regex_match_reached="
             << (state.reachedRegexMatch ? "true" : "false") << "\n";
      rdFile << "regex_functions=";
      {
        bool firstFunc = true;
        for (const auto &fn : state.reachedRegexFunctions) {
          if (!firstFunc) rdFile << ",";
          rdFile << fn;
          firstFunc = false;
        }
