...
///
/// RegexGuidedSearcher — prioritize states that reached deeper regex phases
///

// Static phase lookup: function name → phase (1-4)
static int regexFuncPhase(const std::string &name) {
  // Phase 4: match_post + cleanup
  if (name == "re_copy_regs" || name == "set_regs" || name == "update_regs" ||
      name == "match_ctx_clean" || name == "match_ctx_free" ||
      name == "rpl_regfree" || name == "regfree" || name == "dfafree")
    return 4;

  // Phase 3: match
  if (name == "regexec" || name == "rpl_regexec" ||
      name == "re_search" || name == "rpl_re_search" ||
      name == "re_search_2" || name == "rpl_re_search_2" ||
      name == "re_match" || name == "rpl_re_match" ||
      name == "re_match_2" || name == "rpl_re_match_2" ||
      name == "check_matching" || name == "match_ctx_init" ||
      name == "summarize_regexp_list" || name == "match_slash" ||
      name == "regexp" || name == "fnmatch" ||
      name == "found_in_list" || name == "strstrwrapper" ||
      name == "quote_length" || name == "proc_text" || name == "proc_footer" ||
      name == "dfaexec")
    return 3;

  // Phase 2: compile
  if (name == "regcomp" || name == "rpl_regcomp" ||
      name == "regex_compile" || name == "compile_regex" ||
      name == "compile_regex_1" || name == "initialize_regex" ||
      name == "extract_regexp" || name == "process_regexp" ||
      name == "add_regexp" || name == "make_regexp" ||
      name == "make_regnode" || name == "compile" || name == "regexp_init" ||
      name == "dfacomp" || name == "dfaparse" || name == "dfasyntax")
    return 2;

  // Phase 1: compile_early
  if (name == "parse_reg_exp" || name == "register_state" || name == "setmatcher")
    return 1;

  return 0;
}

int RegexGuidedSearcher::getRegexPhase(const ExecutionState &es) {
  int maxPhase = 0;
  for (const auto &fn : es.reachedRegexFunctions) {
    int p = regexFuncPhase(fn);
    if (p > maxPhase) maxPhase = p;
    if (maxPhase == 4) break;  // already max
  }
  return maxPhase;
}

RegexGuidedSearcher::RegexGuidedSearcher(Searcher *_baseSearcher, double _guideRatio)
    : baseSearcher(_baseSearcher), guideRatio(_guideRatio) {}

RegexGuidedSearcher::~RegexGuidedSearcher() {
  delete baseSearcher;
}

ExecutionState &RegexGuidedSearcher::selectState() {
  // CompoSE: No guiding — phase-0 seed states already removed in Executor::run().
  // Base searcher handles all state selection during normal execution.
  return baseSearcher->selectState();
}

void RegexGuidedSearcher::update(ExecutionState *current,
                                  const std::vector<ExecutionState *> &addedStates,
                                  const std::vector<ExecutionState *> &removedStates) {
  // Maintain our state set
  for (auto *es : addedStates)
    allStates.insert(es);
  for (auto *es : removedStates)
    allStates.erase(es);

  // Forward to base searcher
  baseSearcher->update(current, addedStates, removedStates);
}