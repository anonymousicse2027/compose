...
cl::opt<double> RegexGuideRatio(
    "regex-guide-ratio",
    cl::desc("Probability of regex-guided state selection (0.0=disabled, "
             "0.5=half guided/half base). Only active when --guide-by-regex "
             "is also set. (default=0.5)"),
    cl::init(0.5),
    cl::cat(SearchCat));
} // namespace

// Defined in Executor.cpp
extern llvm::cl::opt<bool> GuideByRegex;
...
Searcher *klee::constructUserSearcher(Executor &executor) {
  ...
  // Regex-guided searcher: wrap the base searcher to prefer states
  // that have reached deeper regex processing phases
  if (GuideByRegex && RegexGuideRatio > 0.0) {
    searcher = new RegexGuidedSearcher(searcher, RegexGuideRatio);
  }
