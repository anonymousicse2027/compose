...
namespace klee {
 ...
 //For Learch
  ...
  /// Regex-guided searcher: wraps a base searcher and preferentially selects
  /// states that have reached deeper regex processing phases.
  /// Phase hierarchy (from data analysis):
  ///   0 = no regex function reached
  ///   1 = compile_early (parse_reg_exp, register_state, setmatcher)
  ///   2 = compile (regcomp, regex_compile, compile_regex, ...)
  ///   3 = match (regexec, check_matching, regexp, ...)
  ///   4 = match_post (re_copy_regs, set_regs, match_ctx_clean, ...)
  class RegexGuidedSearcher : public Searcher {
  public:
    RegexGuidedSearcher(Searcher *baseSearcher, double guideRatio = 0.5);
    ~RegexGuidedSearcher();
    ...
    void update(ExecutionState *current,
                const std::vector<ExecutionState *> &addedStates,
                const std::vector<ExecutionState *> &removedStates);
    bool empty() { return allStates.empty(); }
    void printName(llvm::raw_ostream &os) {
      os << "<RegexGuidedSearcher> guideRatio=" << guideRatio
         << ", baseSearcher:\n";
      baseSearcher->printName(os);
      os << "</RegexGuidedSearcher>\n";
    }

  private:
    Searcher *baseSearcher;
    double guideRatio;  ///< probability of regex-guided selection vs base

    /// All tracked states
    std::set<ExecutionState *> allStates;

    /// Compute the maximum regex phase for a state (0-4)
    static int getRegexPhase(const ExecutionState &es);
  };
}
