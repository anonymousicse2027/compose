...
from featmaker_subscript import klee_executor_without_optimize
...
if __name__=="__main__":
    ...
    parser.add_option(
        "--pgm",
        dest="pgm",
        help="Benchmarks : csplit, diff, expr, find, gawk, grep, ptx, sed, nano, tac, nl, m4",
        choices=["csplit", "diff", "expr", "find", "gawk", "grep", "ptx" ,"sed","nano","tac", "nl","m4"]
    )
    parser.add_option(
        "--output_dir",
        dest="output_dir",
    ...
    pconfig = load_pgm_config(config_file)
    for key in ["pgm_dir", "gcov_path", "src_file"]:
        if key in pconfig:
            pconfig[key] = _resolve_cfg_path(pconfig[key], _cfg_root)
    llvm_dir = os.path.abspath(pconfig["pgm_dir"])
    os.system(f"cp -r {llvm_dir} {top_dir}/")

    if pgm == "m4":
        ke = klee_executor_without_optimize.klee_executor(pconfig, top_dir, options)
    else:
    ...
    while time.time() - start_time < options.total_time:    
        ...
        if iteration != 0:
            ...
            if iteration >= 2:
                os.system(f'find "{top_dir}/result/iteration-{iteration-2}" -type f -name "*.ktest_gcov" -delete')
    ...
    dg.generate_data(iteration - 1) 
    os.system(f'find "{top_dir}/result/iteration-{iteration-1}" -type f -name "*.ktest_gcov" -delete')
