...
class klee_executor:
    ...
    def gen_run_cmd(self, iteration, weight_idx , klee_max_time):
        symbolic_args = self.pconfig.get("sym_options", "")
        sym_args = self.pconfig.get("sym_args", "")
        ...
        run_cmd = " ".join([self.bin_dir+"/klee", 
                                    ...
                                    "--posix-runtime", f"-env-file={os.path.join(_FM_ROOT, 'klee', 'test.env')}",
                                    ...
                                    self.pgm+".bc", symbolic_args, sym_args, "1>/dev/null 2>/dev/null"])
        return run_cmd
