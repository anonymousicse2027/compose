...
def get_pc(ktest):
    ...
    # Check the .kquery file exists
    if not os.path.exists(kquery):
        print(f"Warning: {kquery} does not exist.")
        return []

    with open(kquery, 'r', errors='ignore') as f:
        content = f.read()

    # Return an empty list if there is no 'query' block
    if '(query [' not in content:
        print(f"Warning: {kquery} has no 'query' pattern.")
        return []

    # Extract the query block using the new format
    lines = content.split('(query [')

    # Guard against an unexpected (too-short) split result
    if len(lines) < 2:
        print(f"Warning: {kquery} is not in the expected format.")
        return []

    # Drop the trailing `] false)` and return only the query part
    query_lines = lines[1].split('\n')
    query_lines = [line.strip() for line in query_lines if line.strip()]  # drop blank lines

    # Remove the final `false)` line if present
    if query_lines[-1] == 'false)':
        query_lines.pop()
