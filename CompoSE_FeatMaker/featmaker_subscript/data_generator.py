...
class data_generator:
    def __init__(self, pconfig, top_dir, options):
        ...
        self.gcda_list = ' '.join(f for f in self.gcda_list.split() if 'signal.gcda' not in f)
    ...
    def run_replay(self, iteration,widx, potential_errors):
        ...
        for ktest in ktest_lst:
            ...
            gcov_path = f"{ktest}_gcov"
            if os.path.exists(gcov_path):
                covered_branches[ktest] = branch_handler(gcov_path)
            process.kill()
