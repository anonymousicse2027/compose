...
// ── CompoSE: Regex function-based state guiding options ──
cl::opt<bool> GuideByRegex(
    "guide-by-regex",
    cl::desc("Guide state exploration based on regex function reachability (default=false)"),
    cl::init(false));

static unsigned constTestCaseCounter = 0;
...
void Executor::executeMemoryOperation(ExecutionState &state,
  ...
  for (ResolutionList::iterator i = rl.begin(), ie = rl.end(); i != ie; ++i) {
    ...
    ref<Expr> inBounds = mo->getBoundsCheckPointer(originalAddress, bytes);
