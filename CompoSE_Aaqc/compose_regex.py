...
def _ere2bre(p):
    ...
    for c in p:
        ...
        if c == '\\': esc = True; continue
        if ic: o.append(c); ic = (c != ']'); continue
...
def _bre2ere(p):
    ...
    while i < n:
        c = p[i]
        if ic: o.append(c); ic = (c != ']'); i += 1; continue
...
def _too_complex(pgm, s, rm):
    ...
    if L > _MAX_LEN or not _depth_ok(s) or len(_RE_POSIX.findall(s)) > _MAX_POSIX: return True
    if len(_RE_WB.findall(s)) > _MAX_WB or len(_RE_ALTP.findall(s)) > _MAX_ALT or _RE_NQ.search(s): return True
    if pgm in ('tac', 'nl'):
...
def _sanitize(pgm, pat, rm):
    ...
    s = _cap_ds(s); s = _RE_MID.sub('', s); s = _RE_WB.sub('', s)
    if rm == "ere":
        s = _bre2ere(s)
...
def _grep_ok(pat, rm):
    try:
        p = sp.run(["grep", "-E" if rm == "ere" else "-G", "--", pat],
                   input="", universal_newlines=True, stdout=sp.DEVNULL, stderr=sp.DEVNULL)
        return p.returncode != 2
    except: return False
@dataclass
class FragStat:
    n: int = 0; reward_sum: float = 0.0; success: int = 0; fail_compile: int = 0; last_used_iter: int = -1; ewma: float = 0.0
@dataclass
class FragItem:
    ...
    while i < n:
        ...
        if c == '\\':
            ...
            if tk in _EC:
                ...
                continue
            if len(tk) == 2 and tk[0] == '\\' and tk[1] in '123456789':
                nds.append(RNode("BACKREF", i, ee, tk))
                q = _mq(pat, ee)
                if q: nds.append(RNode("QUANT", q[0], q[1], pat[q[0]:q[1]])); i = q[1]
                else: i = ee
                continue
...
class FragmentDB:
    ...
    def _purge(self):
        for k in [k for k, it in self.items.items() if int(getattr(it.stat, "fail_compile", 0)) > 0]: self.items.pop(k, None)
    def _load(self):
        ...
        try:
            ...
            if isinstance(raw, dict) and "items" in raw and "entries" in raw:
                ...
                for trip in (raw.get("entries", []) or []):
                    try:
                        k_, t_, s_ = trip
        ...
        except: self.items = {}; self.entries = []
    def save(self):
        self._purge()
        pickle.dump({"items": {(k, t): {"kind": v.kind, "text": v.text, "stat": asdict(v.stat)} for (k, t), v in self.items.items()},
                     "entries": list(self.entries)}, open(self.path, "wb"))
    def add_from_patterns(self, patterns):
        sk = set(self.items.keys()); ss = {_norm_sig(k, t) for (k, t) in self.items}
        for p in patterns:
            try:
                for nd in parse_nodes(p):
                    sig = _norm_sig(nd.kind, nd.text); key = (nd.kind, nd.text)
                    if sig not in ss and key not in sk: self.items[key] = FragItem(nd.kind, nd.text, FragStat()); sk.add(key); ss.add(sig)
            except: continue
        self._purge()
    ...
    def record_reward(self, k, t, r):
        ...
        self.entries.append((str(k), str(t), float(r)))
        x = self.items.get((k, t))
        if x:
    ...
    def prune(self, ci):
        ...
        for kind, items in bk.items():
            N = sum(max(0, int(it.stat.n)) for it in items); lt = math.log(1.0 + max(0, N))
            sc = sorted([((it.stat.reward_sum / max(1, int(it.stat.n))) + math.sqrt(lt / (1.0 + max(0, int(it.stat.n)))), (it.kind, it.text)) for it in items], key=lambda x: x[0], reverse=True)
        ...
        return len(dl)
    def pick(self, kind, avoid=None, ci=-1, iu=None, self_text=None):
        avoid = set(avoid) if avoid else set()
        if self_text:
            avoid.add(self_text)
        iu = iu or Counter()
        pool = [v for (k, _), v in self.items.items()
                if k == kind and v.text not in avoid and int(getattr(v.stat, "fail_compile", 0)) == 0]
        ...
        for v in pool:
            ...
            bonus = math.sqrt(lt / (1.0 + n)) if lt > 0 else 0.0
            weights.append(max(0.0, mean + bonus) + 1e-9)
        try:
            return random.choices(pool, weights=weights, k=1)[0].text
        except Exception:
            return random.choice(pool).text
def _kpri(nds, fdb, ku):
    del fdb, ku
    return sorted(nds, key=lambda nd: -(MUTATE_KIND_WEIGHTS.get(nd.kind, 40) * random.random()))
def _mutate1(pat, fdb, rm, trials=50, k=1, ci=-1, iu=None, ku=None):
    ...
    for _ in range(trials):
        nds = _kpri(cands[:], fdb, ku); pk = []; prs = []; ops = []; uk = set(); ut = set(); av = {}
        ...
        for pn in (1, 2):
            for nd in nds:
                ...
                a = av.setdefault(nd.kind, set()); r = fdb.pick(nd.kind, avoid=(ut | a), ci=ci, iu=iu, self_text=nd.text)
                ...
                fdb.record_use(nd.kind, r, ci); iu[(nd.kind, r)] += 1; ku[nd.kind] += 1; uk.add(nd.kind); ut.add(r); a.add(r)
            if len(pk) >= k: break
        if not prs: continue
        prs.sort(key=lambda t: t[0].start); o = []; last = 0
...
def _compute_k(par, fr):
    ...
    m = _cnt_nl(nds); return min(max(0, int(math.floor(m * float(fr)))), m) if m > 0 else 0
...
def _pred_quality(ops, fdb):
    ...
    for op in (ops or []):
        ...
        if not it: continue
        n = max(0, int(it.stat.n)); vals.append((it.stat.reward_sum / n) if n > 0 else 0.0)
    return sum(vals) / len(vals) if vals else 0.0
def _frag_eff(ops):
    f = ops.get(OP_FRAGMENT, {"tries": 0, "reward": 0.0})
    r = ops.get(OP_RANDOM,   {"tries": 0, "reward": 0.0})
    f_t = int(f.get("tries", 0)); r_t = int(r.get("tries", 0))
    if f_t == 0 and r_t == 0: return 0.5
    f_e = float(f.get("reward", 0.0)) / max(1, f_t)
    r_e = float(r.get("reward", 0.0)) / max(1, r_t)
    tot = f_e + r_e
    return (f_e / tot) if tot > 0 else 0.5
def _build_rgx_profile(pgm, rm):
    p = {"mode": rm, "slots": {"char": 30, "class": 20, "alt": 8, "anchor": 12, "boundary": 0, "lookaround": 0, "backref": 0},
    ...
    return p
def _gen_rx(mode, profile_path=None):
    with tempfile.NamedTemporaryFile(mode="r+", delete=False) as tf: path = tf.name
    try:
        cmd = ["regexgen", "-c", "1", "--mode", mode, "-f", path]
...
def mutate_once_bug_exploit(pattern, fdb, max_trials=50, k=3, current_iteration=-1):
    ...
    for _ in range(max_trials):
        nodes_with_w=[(nd, MUTATE_KIND_WEIGHTS.get(nd.kind, 40) * random.random()) for nd in base_nodes]
        nodes_with_w.sort(key=lambda x: -x[1])
        nodes=[nd for nd,_w in nodes_with_w]
        ...
        for nd in nodes:
            ...
            repl=fdb.pick(nd.kind,ci=current_iteration,self_text=nd.text)
...
def mutate_once_bug_explore(pattern, fdb, max_trials=50, k=3, current_iteration=-1):
    ...
    for _ in range(max_trials):
        ...
        for nd in nodes:
            ...
            if nd.kind in MUTATE_SKIP_KINDS_BUG:continue
            if any(not(nd.end<=p.start or nd.start>=p.end) for p in picked):continue
            pool=[v for (kk,_),v in fdb.items.items()
                  if kk==nd.kind and v.text!=nd.text and int(getattr(v.stat,"fail_compile",0))==0]
...
__all__ = ['_depth_ok', '_cap_ds', '_pcre_only', '_ere2bre', '_bre2ere', '_too_complex', '_sanitize', '_grep_ok', 'FragStat', 'FragItem', 'RNode', '_norm_sig', '_resc', '_rcc', '_rgrp', '_rq', '_mq', 'parse_nodes', '_cnt_nl', '_uniq_nlk', 'FragmentDB', '_kpri', '_mutate1', '_compute_k', '_mut_cands', '_pred_quality', '_frag_eff', '_build_rgx_profile', '_gen_rx', '_wide_fragment_boost', '_collect_group_count', '_balanced_groups_bug', '_mirror_tail_insert', 'mutate_once_bug_exploit', 'mutate_once_bug_explore']